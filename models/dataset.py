...
class Dataset:
    ...
    def __init__(self, conf):
        ...
        images_lis = None
        for ext in ['.png', '.JPG']:
            # TODO I put ordering from my ngp_mt code
            images_lis = glob(os.path.join(self.data_dir, f'image/*{ext}'))
            images_lis.sort(key=lambda _:int(_.split('/')[-1].split('.')[0]))
            im_idx = list(range(len(images_lis)))
            ...
            if self.which_split == 'train':
                images_lis = images_lis[::self.half_step*2]; 
                self.vec_stem_files = self.vec_stem_files[::self.half_step*2]
                im_idx = im_idx[::self.half_step*2]
            elif self.which_split == 'test':
                images_lis = images_lis[self.half_step::self.half_step*2]
                self.vec_stem_files = self.vec_stem_files[self.half_step::self.half_step*2] 
                im_idx = im_idx[self.half_step::self.half_step*2]
            else:
                raise NotImplementedError
            # TODO TODO TODO TODO TODO TODO TODO TODO TODO TODO TODO TODO 
            if len(images_lis) > 0:
        ...
        if len(masks_lis) == 0:
            ...
            assert len(masks_lis) == len(images_lis) # TODO
        ...
        self.world_mats_np = [camera_dict['world_mat_%d' % idx].astype(np.float32) for idx in im_idx]
        ...
        if self.estimate_scale_mat:
            ...
            self.scale_mats_np = [camera_dict['scale_mat_%d' % idx].astype(np.float32) for idx in im_idx]
        ...
        if self.use_normal:
            ...
            if self.which_split == 'train':
                ...
                normals_npz = normals_npz[self.half_step::self.half_step*2]; 
                stems_normal = stems_normal[self.half_step::self.half_step*2]
            else:
                raise NotImplementedError
            # TODO TODO TODO TODO TODO TODO TODO TODO TODO TODO TODO TODO
            ...
            if debug_ and IOUtils.checkExistence(f'{self.data_dir}/depth'):
                ...
                # TODO TODO TODO TODO TODO TODO TODO TODO TODO TODO TODO TODO
                if self.which_split == 'train':
                    self.depths_np = self.depths_np[::self.half_step*2] 
                    stems_depth = stems_depth[::self.half_step*2]
                elif self.which_split == 'test':
                    self.depths_np = self.depths_np[self.half_step::self.half_step*2]
                    stems_depth = stems_depth[self.half_step::self.half_step*2]
                else:
                    raise NotImplementedError
                # TODO TODO TODO TODO TODO TODO TODO TODO TODO TODO TODO TODO
    ...
    def initialize_patchmatch(self):
        ...
        if self.denoise_gray_image:
            ...
            if not checkExistence(dir_denoise) and len(get_files_stem(dir_denoise, '.png'))==0 or True: # TODO Always do this
                ...
                write_images(dir_denoise, self.images_denoise_np, self.vec_stem_files)
            else:
                ...
                self.images_denoise_np = np.stack([self.read_img(im_name, self.resolution_level) for im_name in images_denoised_lis if im_name in self.images_lis])
        else:
